...
the **VHDL Hardware Description Language (HDL)** and the **XSim simulator** that comes
...
In order to implement and simulate our simple inverter we are going to write **two different VHDL modules**:

* one module containing the actual implementation of the inverter using **synthesizable code**
* a second **testbech module** that **instantiates** the previous entity as **Device Under Test (DUT)** <br />
...
By default the compiler creates a **log file** named `xvhdl.log` in the directory where you invoke the `xvhd` executable.
...
> ```
>
> Why new files created in the working directory such as `Inverter.vhd` or `compile.log` are not recognized by Git as new files for tracking ? <br /><br />
>
>   \____________________________________________________________________________________________________
>
...
******************************:
  ...
  -d [ --define ] arg              Define VHDL macros. Use -d|--define for
                                   each VHDL macro. The format of the macro
  ...
  -i [ --include ] arg             Specify directories to be searched for files
                                   included using VHDL `include. Use
  ...
  -L [ --lib ] arg                 Specify search libraries for the
                                   instantiated design units in a VHDL or
  ...
  --nolog                          Suppress log file generation
  --override_timeunit              Override timeunit for all VHDL modules,
  ...
  --timescale arg                  Specify default timescale for VHDL
 ...
 ***************:
  ...
  --generic_top arg                Override generic or parameter of a top level
                                   ...
                                   -generic_top "P1=10"
  --ignore_assertions              Ignore System VHDL Concurrent Assertions
  --report_assertion_pass          Report System VHDL Concurrent Assertions
                                   Pass, even if there is no pass action block
  --ignore_coverage                Ignore System VHDL Functional Coverage
  ...
  --timeprecision_vhdl arg (=1ps)  Specify time precision for vhdl designs(
                                   Default: 1ps)
  --transform_timing_checkers      Transform timing checkers to VHDL
                                   processes

 timing simulation:
 *****************:
  --maxdelay                       Compile VHDL design units with maximum
                                   delays
  --mindelay                       Compile VHDL design units with minimum
                                   delays
  --nosdfinterconnectdelays        Ignore SDF port and interconnect delay
                                   constructs in SDF
  --nospecify                      Ignore VHDL path delays and timing checks
  --notimingchecks                 Ignore timing check constructs in VHDL
                                   specify block(s)
  ...
  --typdelay                       Compile VHDL design units with typical
                                   delays (Default)
 ...
 ************:
  --dup_entity_as_module           Enable support for hierarchical references
                                   inside the VHDL hierarchy in mixed
...
As you can notice the simulator informs you that the simulation runs until the `$finish` VHDL task is found in the testbench code.
...
Open the `Inverter.vhd` file with your text editor application and modify the **continuous assignment** in order
