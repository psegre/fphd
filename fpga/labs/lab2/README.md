...
In this second lab we introduce basic **VHDL bitwise logic operators** to implement fundamental **logic gates**
...
* introduce VHDL bitwise logic operators
...
With a **text editor** application create a new file named `Gates.vhd`, for example:
...
in Tcl with such a technique, but this choice is less popular than using a testbench written in VHDL or VHDL.
...
Create a new VHDL file `RingOscillator.vhd` and try to implement and simulate a simple **ring oscillator** circuit as in figure.
One possible coding style is to implement the circuit in form of a **gate-level schematic**
using VHDL logic operators `nand` and `not` with **propagation delays**. 
...
Create also a suitable testbench file `tb_RingOscillator.vhd` to drive the `start` signal of the ring-oscillator and simulate
