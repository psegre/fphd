...
* understand the difference between _Project Mode_ and _Non Project Mode_ in Vivado
...
Explore the content of the `Makefile` using `cat`, `more` or `less` at the command line or
with a text editor as preferred:
...
```
add_files -norecurse /path/to/fphd/fpga/labs/lab3/Gates.vhd
...
to add the VHDL testbench `tb_Gates.vhd` to simulation sources.
...
```
add_files -fileset sim_1 -norecurse /path/to/fphd/fpga/labs/lab3/tb_Gates.vhd
