...
* review how to implement a free-running counter in VHDL
...
Create with your **text-editor** application a new VHDL file named `rtl/LED_blink.vhd` as follows:

```
% gedit rtl/LED_blink.vhd &   (for Linux users)

% n++ rtl\LED_blink.vhd       (for Windows users)
```
...
entity LED_blink is
   ...
   clk       : in  std_logic ;      -- assume 100 MHz external clock from on-board oscillator
   LED       : out std_logic ;
...
architecture rtl of LED_blink is
      ...
      signal count ... := (others => '0') ;     -- declare the counter and initialize the counter to the desired value
                                                -- (choose yourself an appropriate size for the counter)
begin
...
In the proposed code skeleton the `(others => '0')` statement has been used to **initialize the free-running counter** to some default value.
As an example, the following code initializes to zero all FlipFlop outputs in the counter:

```vhdl
signal count : std_logic_vector(27 downto 0) := (others => '0') ;   -- initialize the counter to all-zeroes
```
...
Modify the `LED_blink.vhd` code and **comment-out** the initialization of the free-running counter:
...
Modify the `LED_blink` module in order to add an `enable` input port to be used as external
