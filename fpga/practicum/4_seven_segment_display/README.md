...
exist for this task (such as the **CMOS 4511** or **TTL 7447**) you will implement the decoder in FPGA using VHDL.
...
* write a suitable combinational block in VHDL to drive a 7-segment display
...
```
% cd Desktop/fphd/fpga/practicum/4_seven_segment_display
```
...
Create with your **text-editor** application a new **VHDL file** named `SevenSegmentDecoder.vhd` as follows:

```
% gedit SevenSegmentDecoder.v &   (for Linux users)

% n++ SevenSegmentDecoder.vhd     (for Windows users)
``` 

<br />

The `SevenSegmentDecoder` VHDL module that you are going to implement has **4-inputs** corresponding to a 4-bit binary code
interpreted as BCD, e.g. `BCD[3:0]` or `Bin[3:0]` and **8-outputs** to drive the LEDs of the 7-segment display. e.g. `seg[7:0]`
or `segA`, `segB` etc. If you want you can also decide to simply tie-down the decimal point (DP) LED of the display.
...
As an example, the main `entity` declaration could be the following: 
...
Try to **write yourself** synthesizable VHDL code that implements this functionality. Remind that you can **check for syntax errors**
at any time by **compiling the code** with `xvhdl` at the command line:

```
% xvhdl SevenSegmentDecoder.vhd
```
...
> **HINT**
>
> The block is a pure **combinational circuit**, therefore you can use a **truth-table** implemented using a VHDL `case` statement.
...
In order to map the VHDL code on real FPGA hardware you also need to write a **constraints file** using a
...
As already discussed in `lab3` Xilinx Vivado supports two different usage modes, namely _**Project Mode**_ and _**Non-Project Mode**_.
...
import VHDL and XDC sources. Moreover no on-disk project is created by Vivado, in fact _Non-Project Mode_ flows
